...
title: 深入理解 Radix UI
date: 2023-3-17
summary: 在 Radix UI 之上创作的组件库 shadcn/ui 是今年前端最火的组件库，发布两个多月以来，已经有 10k+ star。本文将带你深入理解 Radix UI，让你创作出像 shadcn/ui 一样的组件并使用它。
tags:
  - Tailwind CSS
  - Radix UI
last_modified_at: 2023-3-30
---
...
Radix UI 是一个 low level 的组件库，它提供了一系列无样式的基础组件，这些组件提供了灵活、强大的 API，让你可以根据自己的需求组合和扩展组件，可以帮助你快速构建高质量、可访问的设计系统和 Web 应用程序。

想要理解 Radix UI，首先要了解前端组件设计中两个非常重要的设计模式。
...
复合模式 (Compound Pattern) 旨在创建多个组件，协同完成一个任务。这些组件可以灵活地组合和配置，而不需要传递复杂的 props。

Compound Pattern 通常使用 React 的 Context API 来实现，通过 Provider 组件来提供状态和方法给子组件。子组件可以使用 useContext hook 来访问 Provider 提供的值。
...
- MovieSelect 文件是一个容器组件，它使用某个数据获取 Hook 获取了数据，并将数据传递给了展示组件。
...
做了张图，方便理解 Radix UI 的 Select 组件，其他组件的结构也有很多相似之处。

<Image
  src={"/images/deep-dive-into-radix-ui/select-demo-dark.png"}
  width={4176}
  height={2589}
  alt="select-demo-dark"
...
- `Select.Root`: 没有 HTML 元素实体。可以理解为控制器，包含 Select 组件的状态，还有相关 `onXXX` 事件。既能可控(直接传入 `value` 和 `open`)，也能不可控(使用 `defaultValue` 和 `defaultOpen`)。我们使用这个子组件来和业务逻辑进行交互，最重要的一个部分。

- `Select.Portal`: 没有 HTML 元素实体。使用 react-dom 中的 `createPortal` API 将选择器的内容传送到 `document.body` 下，这样就不会受到父元素的样式影响。比如如果你没有使用 Portal，那么你的选择器就会受到父元素的 flex 布局影响，造成意想不到的结果，一般都会使用 Portal。但是配合 storybook 中进行测试的时候，会传送出 canvas，导致使用 testing-library 的 Query API 无法获取到元素，这个时候要选择使用 `import { screen } from "@storybook/testing-library"` 来替换 play 函数中的 `canvasElement` 获取元素。
...
- `Select.Icon`: 这个图标通常可以固定，一个向下的小箭头，直接封装进 `Select.Trigger`。
...
export function FoodSelect({ userId }: { userId: number }) {
  const { data: foodList } = useFoodListData();
    ...
    >
      ...
      <SelectContent>
        ...
        <SelectGroup>
          ...
          {foodList.vegetables.map((vegetable) => (
