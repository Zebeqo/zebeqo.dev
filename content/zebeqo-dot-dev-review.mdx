...
2. Static Rendering 中预渲染的页面包括布局和留言板的 Skeleton UI 以及用户相关 UI 的 Skeleton，然后等待 Client-Side `fetch` 获取数据后渲染留言板和客户端。Dynamic Rendering 中，客户端第一次收到的页面是渲染的布局和用户相关 UI 和留言板 Skeleton UI，然后等待服务器发送渲染好的留言板 hydrate。Dynamic Rendering 中用户相关 UI 可以更早加载，因为服务器端第一次接受用户请求的时候就能获取用户验证信息渲染了。
