...
2. Static Rendering 中预渲染的页面包括布局和留言板的 Skeleton UI 以及用户相关 UI 的 Skeleton，然后等待 Client-Side `fetch` 获取数据后渲染留言板和客户端。Dynamic Rendering 中，客户端第一次收到的页面是渲染的布局和用户相关 UI 和留言板 Skeleton UI，然后等待服务器发送渲染好的留言板 hydrate。Dynamic Rendering 中用户相关 UI 可以更早加载，因为服务器端第一次接受用户请求的时候就能获取用户验证信息去渲染用户 UI 了。
...
假如我的网站主页 `/home` 也是 Dynamic Rendering，然后需要展示留言板的最新留言，当用户在我的留言板页面 `/guestbook` 完成留言，对 `POST` API 发起请求，刷新完当前页面后，`/guestbook` 的留言板确实是最新的了，但是在使用 Next.js 框架的客户端导航组件 `<Link>` 的时候网站主页的最新留言是不会更新的。有一个比较 rough 的解决方案是把所有通往主页的客户端导航组件 `<Link>` 全部改成原始的 `<a>` ，像第一次加载一样加载主页。但是这种方法会丢失所有的客户端状态，重新 hydrate 整个页面。遇到这种像数据 span 多个页面，复杂一点情况的，我觉得现阶段要么补上 `GET` API 然后换成 Static Rendering with Client-Side `fetch`，要么等即将推出的 [Revalidating components API](https://vercel.com/blog/vercel-cache-api-nextjs-cache#revalidating-components-with-cache-tags)。
